...
namespace ShowRichResources
{
    ...
    {
        //新的选项参数接口需要先声明，注意它只是接口，不是直接用于设置的实际参数。想要实际使用或赋值时，需要加上“.value”
        //ConfigEntry<Type>括号内的数值类型（Type）自己填，可以填已有的、也可以填写自设数据类型
        ...
        public ConfigEntry<bool> cityHalve;
        ...
        //（新建一个Config配置文件 获取公开/设定私有？不懂）
        public ConfigFile Config { get; private set; }

        //默认会先读取 BepInEx\config\ 文件夹下对应的 .cfg 配置文件（例如本MOD则为：TaiwuMOD.ShowRichResources.cfg），然后初始化配置文件
        //（似乎有方法可以更改读取路径、以及可读取多个配置文件。但这块俺不懂）
        /// <summary>设置选项初始化</summary>
        public void Init(ConfigFile Config)
        {
            //将选项参数接口与配置文件绑定，并填入缺省值
            enabled = Config.Bind("ShowRichResources/显示丰富资源", "enable", true, "【MOD开启】");

            //若读取到配置文件的enable为true时，手动加载Patch（该方式需要YanCore作为支持）
            if (enabled.Value) 
            ...
            modClaimInfo = Config.Bind("ShowRichResources/显示丰富资源", "许可声明请看上方", "", "ShowRichResources/显示丰富资源\n\n原作者：xiaoye1024\n原许可协议声明：MIT许可\n\nBIE版迁移作者：aloneangel34\nBIE版许可协议声明：MIT许可\n");
            ...
            banDestroied = Config.Bind("开关设置项", "banDestroied", false, "【不显示天灾破坏地格】\n是否屏蔽处于被破坏状态地块的资源丰富图标");
            cityHalve = Config.Bind("开关设置项", "cityHalve", false, "【门派城镇采集修正】\n在判定门派城镇（聚落）地格显示资源丰富图标时、是否将其资源数值减半后再计算是否符合阈值\n主要用于查找村民采集地点的时候：\n聚落地块派遣村民采集需要消耗2人力，计算聚落地块平均每人力的采集量时需要除以2");
